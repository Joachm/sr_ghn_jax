...
def main(argv: list[str] | None = None) -> int:
    ...
    config_template = resolve_mujoco_playground_config(
        args.env_id,
        run_preset=args.run_preset,
        overrides={k: v for k, v in base_overrides.items() if v is not None},
    )
    if args.print_config:
        print_resolved_config(
            config_template,
            family="control.mujoco_playground_suite",
    ...
    for seed in seeds:
        config = resolve_mujoco_playground_config(
            args.env_id,
            run_preset=args.run_preset,
            overrides={
                **{k: v for k, v in base_overrides.items() if v is not None},
                "seed": seed,
            },
        )
